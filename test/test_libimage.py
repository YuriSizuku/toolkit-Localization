...
class TestTileImage(unittest.TestCase):
    def test_file_it(self):
        ...
        datasize = ntiletotal*tileinfo.bpp
        
        # test encode decode without palatte
        for i in range(1):
            start = time.time()
            img = libimage.decode_tile_image(data, tileinfo=tileinfo, ntiletotal=ntiletotal)
            print(i+1, f"{(time.time()-start)*1000: .4f} ms for decode_tile_img with {tileinfo}")
            start = time.time()
            data2 =libimage.encode_tile_image(img, tileinfo=tileinfo, ntiletotal=ntiletotal)
            ...
            start = time.time()
            img = libimage.decode_tile_image(data, tileinfo=tileinfo, palette=palatte, ntiletotal=ntiletotal)
            print(i+1, f"{(time.time()-start)*1000: .4f} ms for decode_tile_img with {tileinfo} (with palatte)")
            start = time.time()
            data2 =libimage.encode_tile_image(img, tileinfo=tileinfo, palette=palatte, ntiletotal=ntiletotal)
